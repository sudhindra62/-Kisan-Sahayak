...
import { type ChatbotInput, ChatbotInputSchema, FarmerProfileInputSchema, CentralReliefScheme, CentralReliefSchemeSchema } from '@/ai/schemas';
...
const findCentralReliefScheme = ai.defineTool(
  {
    ...
    description: 'Finds relevant central government relief schemes for a farmer facing distress like crop damage or financial hardship.',
  ...
  async ({ farmerProfile, userQuery }) => {
    ...
    const distressKeywords = ['damage', 'loss', 'flood', 'drought', 'disaster', 'emergency', 'help', 'stress', 'low income', 'rejected'];

    const isDistressQuery = distressKeywords.some(keyword => query.includes(keyword));
    if (!isDistressQuery) {
      return [];
    }
