...
import { ExternalLink, Info, Leaf, Bot, FileText, BookCheck, Clock, AlertTriangle, Phone, ThumbsUp, ChevronsRight, WifiOff, FileBadge } from "lucide-react";
...
import SubsidyClaimButton from "./subsidy-claim-button";
        ...
        <>
          ...
          <div className="space-y-8">
            {results.eligible_schemes.map((scheme, index) => (
              <div className="result-card" key={index} style={{ borderLeft: '6px solid #d6a93c' }}>
                <div className="result-card-header">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-8 pt-6 border-t border-white/10">
                   <div className="flex flex-col gap-4">
                        <Accordion type="single" collapsible className="w-full guide-accordion">
                            ...
                            <AccordionContent className="guide-accordion-content">
                                {generatedGuides[scheme.scheme_name] ? (
                                    ...
                                    <div className="p-4 text-center text-slate-400">
                                        {isOnline ? 'Click to generate a personalized guide.' : 'Connect to the internet to generate the guide.'}
                                    </div>
                                ) : null}
                            </AccordionContent>
                        </AccordionItem>
                        ...
                        <a href={scheme.applicationGuideLink} target="_blank" rel="noopener noreferrer" className="premium-btn-secondary">
                   ...
                   <div className="bg-white/5 p-6 rounded-2xl border border-white/10">
                        <h4 className="flex items-center text-amber-400 font-semibold mb-4">
                            <FileBadge className="mr-2 h-5 w-5" />
                            Instant Subsidy Claim
                        </h4>
                        <p className="text-sm text-white/70 mb-4">
                            Automated document generation and verification for immediate government submission.
                        </p>
                        {user && farmerProfile && (
                            <SubsidyClaimButton 
                                scheme={scheme} 
